...
import com.spider.service.impl.One58MovieListProcessServiceImpl;
...
@Data
@RestController
@Api(tags = "158资源", description = "有关www.158zyz.com网站的爬取")
@CrossOrigin
public class One58Controller {
    ...
    @Autowired
    One58MovieListProcessServiceImpl zd;


    String baseurl = "http://158zyz.com";
    ...
    //从第一页到最后一页爬取
    @GetMapping("one58/climb/{pageIndex}")
    public void climbMovie(@PathVariable(value = "pageIndex") int pageIndex) {
        File file = LogUtil.creatFile("./158资源爬取日志.txt");
        /*起始页数*/
        int index = pageIndex;
            while (true){
...
//                    System.err.println("爬取该电影信息超时" + movieUrl);
            ...
            int length1 = 0;
            if (movie.getMc().getOne58collectionm3u8() != null) {
                length1 = movie.getMc().getOne58collectionm3u8().split(",").length;
            }
            ...
            length = byMovieNameLike.getMc().getOne58collectionm3u8().split(",").length;
            /*ES中不存在该电影，直接向MySql和Elasticsearch中添加该电影*/
            if ("".equals(byMovieNameLike.getMovieName()) && "".equals(byMovieNameLike.getMc().getOne58collectionm3u8())) {
                SaveControllerUtil saveControllerUtil =new SaveControllerUtil(movieESDao,movieMapper,movie,file);
                saveControllerUtil.setName("158");
...
//                LogUtil.fileWriter(file, sdf.format(System.currentTimeMillis()) + ":添加电影id=" + movie.getMovieId() + "：" + movie.getMovieName());
            ...
            /*ES中存在该电影*/
                ...
                if (length1 > length||"".equals(byMovieNameLike.getMc().getOne58collectionm3u8())) {

                    UpdateControllerutil controllerutil=new UpdateControllerutil(movieESDao,movieMapper,movie,file,name,"158");
                    controllerutil.setName("158");
...
//                        System.err.println(sdf.format(System.currentTimeMillis()) + ":更新电影（剧集改变）id=" + byMovieNameLike.getMovieId() + "：" + movie.getMovieName());
                        ...
                        System.out.println(sdf.format(System.currentTimeMillis()) + "\t158:该电影存在id=" + byMovieNameLike.getMovieId() + "：" + movie.getMovieName());
...
//        String url = "http://158zyz.com/?m=vod-index-pg-466.html";
        String url = "http://158zyz.com/?m=vod-detail-id-22196.html";
//        String url = "http://123ku.com/?m=vod-detail-id-34173.html";
//        String url = "http://123ku.com/?m=vod-index-pg-709.html";



        ProcessService movieListProcessService = new One58MovieListProcessServiceImpl();
