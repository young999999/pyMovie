...
import com.spider.service.impl.One23MovieListProcessServiceImpl;
...
@Data
@RestController
@Api(tags = "123资源", description = "有关www.123ku.com网站的爬取")
@CrossOrigin
public class One23Controller {
    ...
    @Autowired
    One23MovieListProcessServiceImpl zd;


    String baseurl = "http://123ku.com";

    static SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");


    //从第一页到pageIndex页更新
    @ApiOperation(value = "123资源更新", notes = "更新123资源，startIndex起始页，endIndex终止页（不包括）")
    @GetMapping("one23/update/{startIndex}/{endIndex}")
    public String updateMovie(@PathVariable(value = "startIndex") int startIndex, @PathVariable(value = "endIndex") int endIndex) {
        File file = LogUtil.creatFile("./123资源更新日志.txt");
    ...
    //从第一页到最后一页爬取
    @GetMapping("one23/climb/{pageIndex}")
    public void climbMovie(@PathVariable(value = "pageIndex") int pageIndex) {
        File file = LogUtil.creatFile("./123资源爬取日志.txt");
        /*起始页数*/
        int index = pageIndex;
        while (true) {
    ...
    public static int update(CommonPageGet commonPageGet, File file, ProcessService processService, String baseurl, int startIndex, MovieESDao movieESDao,MovieMapper movieMapper) {
        {


            String url = baseurl + "/?m=vod-index-pg-" + startIndex + ".html"; //processService
...
//                    LogUtil.fileWriter(file, "下载电影集合页面失败 " + downPageNum + " " + url);
            ...
            if (!processService.processTotlePage(movieListPage, startIndex)) {
                ...
                LogUtil.fileWriter(file, "第" + startIndex + "页没有有电影");
...
//                    LogUtil.fileWriter(file, "爬取该电影信息超时" + movieUrl);
                ...
                int length1 = 0;
                if (movie.getMc().getOne23collectionm3u8() != null) {
                    length1 = movie.getMc().getOne23collectionm3u8().split(",").length;
                }
                ...
                length = byMovieNameLike.getMc().getOne23collectionm3u8().split(",").length;
                /*ES中不存在该电影，直接向MySql和Elasticsearch中添加该电影*/
                if ("".equals(byMovieNameLike.getMovieName()) && "".equals(byMovieNameLike.getMc().getOne23collectionm3u8())) {
                    SaveControllerUtil saveControllerUtil = new SaveControllerUtil(movieESDao,movieMapper, movie, file);
                    saveControllerUtil.setName("123");
...
//                LogUtil.fileWriter(file, sdf.format(System.currentTimeMillis()) + ":添加电影id=" + movie.getMovieId() + "：" + movie.getMovieName());
                ...
                /*ES中存在该电影*/
                    ...
                    if (length1 > length || "".equals(byMovieNameLike.getMc().getOne23collectionm3u8())) {

                        UpdateControllerutil controllerutil = new UpdateControllerutil(movieESDao,movieMapper, movie, file, name, "123");
                        controllerutil.setName("123");
...
//                        System.err.println(sdf.format(System.currentTimeMillis()) + ":更新电影（剧集改变）id=" + byMovieNameLike.getMovieId() + "：" + movie.getMovieName());
                        ...
                        System.out.println(sdf.format(System.currentTimeMillis()) + "\t123:该电影存在id=" + byMovieNameLike.getMovieId() + "：" + movie.getMovieName());
...
//            System.out.println(sdf.format(System.currentTimeMillis()) + ":第" + startIndex + "页完成");
    ...
    public static void main(String[] args) throws IOException {
        ...
        /*酷云资源测试*/
//        String url = "http://158zyz.com/?m=vod-index-pg-466.html";
//        String url = "http://158zyz.com/?m=vod-detail-id-22285.html";
        String url = "http://123ku.com/?m=vod-detail-id-34160.html";
//        String url = "http://123ku.com/?m=vod-index-pg-709.html";


        ProcessService movieListProcessService = new One23MovieListProcessServiceImpl();
